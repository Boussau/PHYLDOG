	  ...
	  for (vector<Node *>::iterator it = leaves.begin(); it!=leaves.end(); it++ )
	    {
	      ...
	      (*it)->setName(tokens[0]);
	      name = (*it)->getName();	      
        /*sp = (dynamic_cast<const BppString*>((*it)->getNodeProperty("S")))->toSTL();
        ur_sigma[name]=ur_sigma[sp];*/
        tokens.clear();
	      Tokenize(name,tokens,gene_token);	     
	      ur_sigma[name]=ur_sigma[tokens[0]];
	      //gene names for Bastien 
	    }
