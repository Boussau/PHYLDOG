...
int main (int argc, char *  argv[]) 
{
  ...
  if (rank==server) 
    { 
      ofstream cout_stream(outstream_file_name.c_str(),ios::app);  
      cout_stream << ll << " 00 Hest " << t->elapsed()<<endl;
      cout_stream.close();
    }
  */  
  if (rank==server) cout << Sstring << endl;
  scalar_type ll=LL_mpi(infer_tree,Sstring,"Hest");
  if (rank==server)
    {
      ofstream cout_stream(outstream_file_name.c_str(),ios::app);  
      cout_stream << ll << " 00 Hest " << t->elapsed()<<endl;
      cout_stream.close();
      cout << ll << " 00 Hest " << t->elapsed()<<endl;
  ...
  if (rank==server)
    {
      ofstream cout_stream(outstream_file_name.c_str(),ios::app);  
      cout_stream << ll << " 00 IHest " << t->elapsed()<<endl;
      cout_stream.close();
      cout << ll << " 00 IHest NC5" << t->elapsed()<<endl;
      
    }
  if (rank==server) infer_tree->print_rates();

  infer_tree->nclasses=3;
  ll=LL_mpi(infer_tree,Sstring,"IHest");
  if (rank==server)
    {
      ofstream cout_stream(outstream_file_name.c_str(),ios::app);  
      cout_stream << ll << " 00 IHest " << t->elapsed()<<endl;
      cout_stream.close();
      cout << ll << " 00 IHest NC3" << t->elapsed()<<endl;
      
    }
  if (rank==server) infer_tree->print_rates();

  infer_tree->nclasses=10;
  ll=LL_mpi(infer_tree,Sstring,"IHest");
  if (rank==server)
    {
      ofstream cout_stream(outstream_file_name.c_str(),ios::app);  
      cout_stream << ll << " 00 IHest " << t->elapsed()<<endl;
      cout_stream.close();
      cout << ll << " 00 IHest NC10" << t->elapsed()<<endl;
