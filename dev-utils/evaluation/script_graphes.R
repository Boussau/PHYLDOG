...
cstart = read.table("summary_content_deco_starting")
dstart = read.table("summary_degres_deco_starting")
crec = read.table("summary_content_deco_reconciled")
# densembl = read.table("summary_degres_deco_ensemblTrees")
drec = read.table("summary_degres_deco_reconciled")
...
boxplot(cextant$V2,crec$V2,cstart$V2,names = c("extant","reconciled","starting"),ylab="Number of genes")
dev.off()

lk_total = read.csv("./lk_before_and_after.csv",h=F)
