...
helpText = ["Welcome to the PHYLDOG file preparation script", "\nThis script will ask you for two input directories, one containing only alignment files, and one containing only link files. Link files relate sequences in the alignments to species. They are formatted as follows:\n\n\tSpecies_A:gene_A1\n\tSpecies_B:gene_B1;gene_B2\n\tSpecies_A:gene_A2\n\tSpecies_C:gene_C1\n\t...\n" , "\nThe script will ask you for a folder that will contain option files, and another that will contain results output by PHYLDOG." + "\nThis script does not intend to cover all the options that PHYLDOG can accept. Rather, it is meant to be a convenient way to quickly generate a list of reasonable option files, with default values that should work in the general case. In particular, this script is pretty dumb about the models of sequence evolution it can set up, and it sets up the same model for all the gene families. PHYLDOG can work with a much larger array of models, including different models for different gene families, but to use them you will need to edit the option files using a text editor or custom scripts.\n\n" , "\nFor comments and questions, please contact Bastien Boussau:\n\n\tboussau@gmail.com\n\n" + "\n"]

def helpbox(text,widthp=60):
    firstParagraph = True
    print("\n ╔"+ (widthp-3)*"═"+"╗"),
    for paragraph in text:
        if not firstParagraph:
            print("\n ╟"+ (widthp-3)*"─"+"╢"),
        for paragraphLines in paragraph.split("\n"):
            for line in textwrap.wrap(paragraphLines,widthp-5):
                print("\n ║ " + (line.center(widthp-5) if firstParagraph else line.ljust(widthp-5))+" ║"),
...
if __name__ == '__main__':
    ...
    #Data type
    DATATYPE = raw_input("What alphabet? [DNA, RNA, CODON, or PROTEIN]\n> ").upper()
    if (not ( "DNA".startswith(DATATYPE) or "RNA".startswith(DATATYPE) or "CODON".startswith(DATATYPE) or "PROTEIN".startswith(DATATYPE) ) ): 
        while (not ( "DNA".startswith(DATATYPE) or "RNA".startswith(DATATYPE) or "CODON".startswith(DATATYPE) or "PROTEIN".startswith(DATATYPE) ) ) :
            ...
            DATATYPE = raw_input("What alphabet? DNA or CODON or PROTEIN?\n> ").upper()
    ...
    #File format
    DATAFORMAT = raw_input("What alignment format? FASTA or MASE or PHYLIP or CLUSTAL or DCSE or NEXUS?\n> ").upper()
    ...
    FILTERING = raw_input("Do you want to filter the alignments and trees using PHYLDOG or do you trust your data is good to go?\n> ").upper()
    while (not ( "YES".startswith(FILTERING) or "NO".startswith(FILTERING) ) ) :
        ...
        FILTERING = raw_input("Do you want to filter the data?\n> ").upper()
    ...
    #Species tree
    TREEFILEGIVEN = raw_input("Do you want to provide a starting species tree (Newick)?\n> ").upper()
    ...
    if ( "YES".startswith(TREEFILEGIVEN) ):
        ...
        STARTINGTREE = raw_input("What kind of species tree? RANDOM or MRP?\n> ").upper()
    ...
    TOPOSPECIES = raw_input("Do you want to optimize the species tree topology?\n> ").upper()
    ...
    DLPARAM = raw_input("Do you want to optimize the duplication and loss parameters?\n> ").upper()
    ...
    if ("YES".startswith(DLPARAM) ):
            DLOPT = raw_input("How do you want to optimize the duplication and loss parameters? Branchwise or average ?\n> ").upper()
    ...
    EQUGENOMES = raw_input("Do you want to assume that all genomes, if they had no missing data, would have the same number of genes?\n> ").upper()
    ...
    INPUTGENETREES = raw_input("Do you want to provide input gene trees?\n> ").upper()
    listTrees = list()
    ...
    TOPOGENE = raw_input("Do you want to optimize the gene tree topologies?\n> ").upper()
    ...
    helpbox("To launch PHYLDOG, you will need to run something like:",
            "\tmpirun -np NUMBER_OF_PROCESSES PATH_TO_PHYLDOG/phyldog param="+ os.path.join(OPTDIR,"GeneralOptions.txt"),
